...
function solve3(M, b) {
  ...
  const Dx = det3([[b[0], M[0][1], M[0][2]], [b[1], M[1][1], M[1][2]], [b[2], M[2][1], M[2][2]]]);
  const Dy = det3([[M[0][0], b[0], M[0][2]], [M[1][0], b[1], M[1][2]], [M[2][0], b[2], M[2][2]]]);
  const Dz = det3([[M[0][0], M[0][1], b[0]], [M[1][0], M[1][1], b[1]], [M[2][0], M[2][1], b[2]]]);
...
export default function TextureBuilder({ capturedFrames, onTextureBuilt }) {
  ...
  // Per-triangle best data
  const bestScoreRef = useRef(null);     // Float32Array
  const bestFrameDataRef = useRef(null); // Array<{lm, img} | null>
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      bestScoreRef.current = new Float32Array(triangles.length).fill(0);
      bestFrameDataRef.current = new Array(triangles.length).fill(null);
  ...
  const redrawCanvas = () => {
    ...
    const bestFrameData = bestFrameDataRef.current;
    if (!triangles || !bestFrameData) return;
    for (let ti = 0; ti < triangles.length; ti++) {
      const fd = bestFrameData[ti];
      if (!fd) continue;
      const { lm, img } = fd;
  ...
  const processQueue = async () => {
    ...
    while (pendingFramesRef.current.length > 0) {
      ...
      setMessage(remaining > 0 ? `Processing... (${remaining + 1} queued)` : "Processing...");
      ...
      if (result?.faceLandmarks?.length) {
        ...
        for (let ti = 0; ti < triangles.length; ti++) {
          ...
          const p0 = lm[srcIndices[0]], p1 = lm[srcIndices[1]], p2 = lm[srcIndices[2]];
          ...
          if (score > bestScore[ti]) {
            bestScore[ti] = score;
            bestFrameData[ti] = { lm, img };
          }
        }
        redrawCanvas();
      }
  ...
  useEffect(() => {
    ...
    if (newCount < prevCount) {
      ...
      bestScoreRef.current = new Float32Array(n).fill(0);
      bestFrameDataRef.current = new Array(n).fill(null);
      redrawCanvas(); // show background only
      if (newCount > 0) enqueue([...capturedFrames]);
    } else {
  ...
  return (
    <div style={{ padding: "16px", color: "#fff", height: "100%", boxSizing: "border-box", overflowY: "auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "12px", flexWrap: "wrap" }}>
        <button className="source-btn" onClick={handleSave} disabled={!ready || processing}>
          💾 Use as Texture
        </button>
        {!ready && <span style={{ color: "#aaa", fontSize: "13px" }}>Initializing...</span>}
        {processing && <span style={{ color: "#aaa", fontSize: "13px" }}>{message}</span>}
        {ready && !processing && (
